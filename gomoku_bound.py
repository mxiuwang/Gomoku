...
def absearch(board, color, preRow, preCol):
    ...
    for row in range((preRow-4)%rows, (preRow+5)%rows):
...
def minimax(board, isMaximizing, color, depth, preRow, preCol):
    rows, cols = board.getBoardSize()
        ...
        for row in range((preRow-4)%rows, (preRow+5)%rows):
            for col in range((preCol-4)%rows, (preCol+5)%cols):
                if board.getColor(row, col) == '.':
                    ...
                    score = minimax(board, False, next_color, depth+1, row, col)
    ...
    # handle Min case 
        ...
        for row in range((preRow-4)%rows, (preRow+5)%rows):
            for col in range((preCol-4)%rows, (preCol+5)%cols):
                if board.getColor(row, col) == '.':
                    board.setColorAtPosition(row, col, color) 
                    score = minimax(board, True, next_color, depth+1, row, col)
                    board.setColorAtPosition(row, col, ".")
                    if score < bestScore:
                        bestScore = score
                        if bestScore == -1:
